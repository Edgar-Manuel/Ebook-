...
5. **El Empaque Fuerte (Optimización de Metadatos A10):** Cómo usar las palabras clave exactas que la gente ya está buscando. Dónde colocarlas (Título, Subtítulo, Autor, y en las 7 casillas de backend KDP). El título debe contener keywords de cola larga con intención de compra. Herramientas de SEO recomendadas: Helium 10 (para análisis de competencia y BSR) y Publisher Rocket (para investigación de keywords y categorías específicas de libros). Recuerda: el título con keywords es para AMAZON (decide dónde rankea tu libro), la portada es para el COMPRADOR (decide si hace clic).
...
11. **Distribución Amplia (Más Allá de Amazon):** Aunque KDP Select (exclusividad Amazon) es ideal al principio por Kindle Unlimited, el autor debe saber que puede expandirse: IngramSpark para libros impresos en librerías físicas y tapas duras (Amazon no ofrece hardcover), Draft2Digital para ebooks en Kobo, Apple Books, Nook, Google Play y bibliotecas. Esto es especialmente valioso cuando ya tienes 3+ libros y quieres maximizar ingresos por múltiples canales.
12. **Checklist Anti-Fracaso antes de Publicar:** Las 8 cosas que debe comprobar con visión de "comprador" antes de darle al botón: portada que diga "este libro es para ti" al lector ideal (no solo bonita), descripción AIDA formateada, keywords de cola larga en backend (sin los 3 errores de 2026), categorías hiper-específicas con competencia manejable, A+ Content profesional, Author Central configurado, back matter estratégico (CTA reseña + siguiente libro + newsletter), declaración de IA correcta.
